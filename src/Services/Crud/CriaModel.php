...
use Tfarias\InstaladorTfarias\Services\TratarCampos;
use File;
...
class CriaModel
{
    ...
    protected $template_uuid = __DIR__.'/CreateCrud/model_uuid.txt';
    protected $base_model = __DIR__.'/CreateCrud/base_model.txt';
    protected $base_uuid = __DIR__.'/CreateCrud/traid_uuid.txt';
    protected $base_mask = __DIR__.'/CreateCrud/traid_clear_masc.txt';
    ...
    public function criar($tabela,$uuid)
    {
        $this->template = $uuid=='n' ? $this->template : $this->template_uuid;
        ...
        if (!File::isFile(base_path('app/Models/Traits/Mask.php'))) {
            $traid = File::get($this->base_mask);
            $traid = str_replace('[{namespace}]', $this->getAppNamespace(), $traid);
            File::put(base_path('app/Models/Traits/Mask.php'), $traid);
        }
        ...
        if (!empty($table)) {
            foreach ($table as $c) {
                ...
                if ((strpos($c->Type, 'decimal') !== false) || $c->Type == 'decimal') {
                    ...
                    $model = str_replace('[{use_currency}]', 'use '.$this->getAppNamespace().'Models\Traits\Currency;', $model);
